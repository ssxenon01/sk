...
var SearchkitManager = /** @class */ (function () {
    ...
    SearchkitManager.prototype._searchWhenCompleted = function (location) {
        ...
        this.registrationCompleted.then(function () {
            _this.searchFromUrlQuery(location);
        }).catch(function (e) {
            console.error(e);
        });
    };
    ...
    SearchkitManager.prototype.searchFromUrlQuery = function (location) {
        var query = history_1.decodeObjString(location.search.replace(/^\?/, ""));
    ...
    SearchkitManager.prototype._search = function () {
        ...
        var queryJson = this.query.getJSON();
        var queryObject = this.queryProcessor(queryJson);
        this.currentSearchRequest && this.currentSearchRequest.deactivate();
        this.currentSearchRequest = new SearchRequest_1.SearchRequest(this.transport, queryObject, this, query);
        return this.currentSearchRequest.run()
            .then(function () {
            return _this.getResultsAndState();
        });
    };
    SearchkitManager.prototype.setResults = function (results, srcQuery) {
        if (srcQuery && srcQuery.shouldAppendResults() && this.results != null) {
            results.hits = assign({}, results.hits, {
                hits: uniqBy(this.results.hits.hits.concat(results.hits.hits), '_id'),
                hasChanged: false
            });
            var mergedResults = assign({}, this.results, {
                hits: results.hits
            });
